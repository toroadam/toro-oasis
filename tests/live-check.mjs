...
const fetchImpl = async url => {
 ...
 const body = names.includes('ControllerStatus') ? rows.status : names.length === 1 ? rows.month : rows.app_open;
 return {ok:true, text:async () => body.map(r => JSON.stringify(r)).join('\n')};
};

const unconfigured = createLive({root:process.cwd(), fetchImpl});
assert.deepEqual(await unconfigured.snapshot(0), {configured:false}, 'no credentials: no Mixpanel calls');
assert.equal(calls.length, 0);

const live = createLive({account:'sa', secret:'pw', project:'4009305', root:process.cwd(), fetchImpl});
...
assert.equal(snap.events.length, 3, 'Testville excluded; the 2-minute-old event is held back by the lag');
assert.deepEqual(snap.events.map(e => e.kind), ['cancelled', 'error', 'activity']);
