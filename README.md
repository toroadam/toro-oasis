...
By default the globe **tours** itself: Network, North America, Northeast, Southwest (10 s each), Australia and India (8 s), back to North America, then each Toro region with its states lit and its panel open, then round again. The headline shows each area's figures as of the replay clock. Picking a chapter pill, dragging, zooming or selecting a place stops the tour; the **Network** pill starts it again.

Select any city, or any event in the feed, to fly there and see its thirty-day totals. When nothing is selected, the right-hand panel shows four network cards (users, users with a controller, controllers added this month, share online). A **Top regions** tab ranks Toro's sales regions (Pacific, Rocky Mountains, Southwest, Midwest, Northeast, Southeast, Noncontiguous, plus Canada and International) by users, controllers added or app activity. Select one to fly there with its states highlighted in the region's colour, then drill into any state. A **Zoom out** button appears above the timeline whenever you are zoomed in close.
...
The site shows a fixed snapshot of Oasis analytics since April 2026 (committed as `public/data/oasis.json`, built with `build-oasis-data.py --long`; each day's counts are spread over its hours by the region's real hour-of-day pattern). **Live** replays the snapshot's last 48 hours as "today and yesterday". No Mixpanel credentials are needed.
