...
export function siteHeader(current){
 ...
 return `<header class="site-header global-header"><a class="wordmark" data-experience="planet" href="${import.meta.env.BASE_URL}" aria-label="Toro Oasis home"><img src="${import.meta.env.BASE_URL}brand/toro-logo.svg" alt="Toro" width="42" height="28"><span class="wordmark-name">Oasis</span></a><nav class="desktop-navigation" aria-label="Main navigation">${items}</nav><div class="header-actions"><div class="desktop-creator">${creator}</div><button id="nav-science" class="header-sources">Sources ${arrowUpRight}</button><button id="menu-toggle" aria-haspopup="dialog" aria-controls="mobile-navigation" aria-expanded="false">Explore <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M3 6h14M3 13h14"/></svg></button></div></header><dialog id="mobile-navigation" aria-label="Explore Toro Oasis"><div class="mobile-menu-heading"><span>TORO OASIS · LIVE NETWORK</span><button aria-label="Close navigation">✕</button></div><nav aria-label="All experiences">${items}</nav><div class="mobile-menu-footer"><button id="mobile-menu-sources" aria-haspopup="dialog">Sources ${arrowUpRight}</button></div></dialog>`;
}
