...
function renderKpis(){
 ...
 kpis.innerHTML=`<header><div class="oasis-tabs" role="tablist"><button role="tab" data-view="network" aria-selected="${kpiView==='network'}">Network</button><button role="tab" data-view="regions" aria-selected="${kpiView==='regions'}">Top regions</button></div><span>${asOf}</span></header>
  ...
  <div class="oasis-kpi"><span>${k.controllerUsers?'Controller users':'Controllers'}</span><strong data-kpi="controllers">${fmt.format(k.controllerUsers??k.controllers)}</strong><small data-kpi="controllersNote">${k.controllerUsers?`Users · ${fmt.format(k.controllers)} controller IDs seen`:k.controllersNote??`Added through the app since ${k.controllersSince}`}</small></div>
...
function growKpis(){
 ...
 set('controllersNote',controllerUserCum?`Users · ${fmt.format(ids)} controller IDs seen${t<controllerTrackedFrom?' (IDs partly logged before June)':''}`:t<controllerTrackedFrom?'Seen in the app. IDs were only partly logged before June':(k.controllersNote??'Connected, all time'));
